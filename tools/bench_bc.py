...
from pymahjong.rl.cached_dataset import CachedEventDataset
...
def build_model(args):
    ...
    from pymahjong.rl.legal_actions import extract_legal_actions
    ...
    def _collate(batch):
        ...
        af, apm, loi, lti = extract_legal_actions(out["action_mask"], action=out["action"])
    ...
    # Tiny stub mimicking the old strategy surface used downstream.
    class _S:
        collate_fn = staticmethod(_collate)

        @staticmethod
        def forward_from_batch(m, b):
            return m(
                b["features"], b["attention_mask"],
                action_features=b["action_features"],
                action_pad_mask=b["action_pad_mask"],
                legal_orig_idx=b["legal_orig_idx"],
            )
