...
only emits V3 token caches). It uses :func:`encode_paipu_file` to
walk each XML, extract per-decision-point V4 samples, and stream them
into :class:`pymahjong.rl.cache.ShardWriter` shards.

Output layout (read by :class:`pymahjong.rl.cached_dataset.CachedEventDataset`)::
