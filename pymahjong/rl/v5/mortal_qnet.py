...
* **Action encoding -> V5 (Douzero)**: each *legal* action's static
  descriptor (:func:`pymahjong.rl.v5.action_features.torch_action_features`,
...
model (:class:`~pymahjong.rl.v5.model.DouzeroV5Transformer`), so a V5 BC
...
from ..v4.model import EventStreamTransformer, NEG_INF
...
class DouzeroQHead(nn.Module):
    ...
    def __init__(
        ...
        # Layout matches DouzeroV5Transformer.scorer for V5 BC warm-start.
...
class MortalQNet(nn.Module):
    ...
    def evaluate_q(
        ...
        Q for the CQL term), ``aux_logits`` ``(B, 4)`` or ``None``, and the
        full legal-action ``q_K`` ``(B, K)`` + ``action_pad_mask`` ``(B, K)``
        (NEG_INF / False on pad) for a BC-policy anchor term.
        """
        h, q_K, action_pad_mask, _legal, target_in_K = self._q_legal(
