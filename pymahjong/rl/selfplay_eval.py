...
Rolling the policy in :class:`MultiAgentEnv` with all four seats
...
def selfplay_eval(
    ...
    *,
    ...
    env = MultiAgentEnv(max_seq_len=max_seq_len)
    fallback_before = MultiAgentEnv._fallback_warned
    ...
    metrics: Dict[str, float] = {
        ...
        "sp/mask_fallback_fired": float(
            MultiAgentEnv._fallback_warned and not fallback_before
        ),
    }
...
def record_one_selfplay_hand(
    ...
    *,
    ...
    loop alongside :func:`selfplay_eval` and you get one watchable
    ...
    model.eval()
    try:
        # Drive 4-seat shared self-play via MultiAgentEnv (same env the
        # main selfplay_eval uses).
        env_inner = MultiAgentEnv(max_seq_len=max_seq_len)
        try:
...
__all__ = ["selfplay_eval", "format_selfplay_metrics", "record_one_selfplay_hand"]
