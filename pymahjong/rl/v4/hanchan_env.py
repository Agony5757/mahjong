"""Hanchan (半庄) wrapper over :class:`V4MultiAgentEnv`.
...
underlying :class:`V4MultiAgentEnv` (and the C++ engine) is a single-hand
...
API mirrors :class:`V4MultiAgentEnv`'s per-kyoku interface but adds the
...
class HanchanEnv:
    """Hanchan-loop wrapper around :class:`V4MultiAgentEnv`.
    ...
    def __init__(
        ...
        *,
        ...
        self._inner: Optional[V4MultiAgentEnv] = None
    ...
    def _start_kyoku(self) -> Dict[str, Any]:
        """Construct a fresh V4MultiAgentEnv at the current bakaze/oya/etc."""
        self._inner = V4MultiAgentEnv(max_seq_len=self.max_seq_len)
        try:
    ...
    def kyoku_step(self, action: int):
        ...
        Returns:
            ...
            :meth:`V4MultiAgentEnv.step`.  ``payoffs`` is the per-kyoku
    ...
    def get_inner_env(self) -> V4MultiAgentEnv:
        if self._inner is None:
            raise RuntimeError("no inner env (call reset() first)")
        return self._inner
    ...
    def _finalize_kyoku(
        ...
        # which V4MultiAgentEnv produced from ``get_payoffs()``.
        res = t.get_result()
