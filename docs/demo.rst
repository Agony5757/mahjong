...
Watch four random AI agents play a complete game (seed=99999). No installation required.

.. raw:: html

   <style>
   #demo-root {{ font-family: system-ui, sans-serif; max-width: 920px; margin: 1.5rem 0; }}
   #demo-canvas {{ display: block; margin: 0 auto; background: #1a2a1a; border-radius: 8px; cursor: default; }}
   ...
   #demo-players {{ display: flex; justify-content: space-around; margin-top: 8px; font-size: 12px; color: #666; }}
   ...
   #demo-speed {{ display: flex; align-items: center; gap: 8px; justify-content: center; margin-top: 6px; font-size: 12px; color: #666; }}
   ...
   function dT(c,x,y,tile,opt) {{
     opt = opt || {{}};
     var w = opt.w || TW, h = opt.h || TH;
     var bg = opt.sel ? '#ffe066' : opt.hl ? '#fff3cd' : '#faf8f0';
     c.shadowColor='rgba(0,0,0,0.28)'; c.shadowBlur=opt.sel?7:3; c.shadowOffsetX=1; c.shadowOffsetY=2;
     c.fillStyle=bg; rRect(c,x,y,w,h,TR); c.fill();
     c.shadowColor='transparent';
     c.strokeStyle=opt.sel?'#f39c12':(opt.hl?'#e67e22':'#aaa'); c.lineWidth=opt.sel?2:1;
     rRect(c,x,y,w,h,TR); c.stroke();
     if(!tile) return;
     c.fillStyle=tc(tile); c.font=`bold ${{w*0.52}}px 'SimHei','Microsoft YaHei',serif`;
     c.textAlign='center'; c.textBaseline='middle';
     c.fillText(tile,x+w/2,y+h/2+1);
   }}

   function dTB(c,x,y) {{
     c.fillStyle='#1a3a5c'; rRect(c,x,y,RW,RH,2); c.fill();
     c.strokeStyle='#3a6a9c'; c.lineWidth=1; rRect(c,x,y,RW,RH,2); c.stroke();
     c.strokeStyle='#2a5a8c'; c.lineWidth=0.5;
     for(var i=-RH;i<RW+RH;i+=5) {{ c.beginPath(); c.moveTo(x+i,y); c.lineTo(x+i+RH,y+RH); c.stroke(); }}
   }}

   function dRT(c,x,y,tile,rot) {{
     if(!tile) {{ dTB(c,x,y); return; }}
   ...
   function dSB(c,i,score,riichi,active) {{
     var lx=[4,W-74,W-74,4], ly=[P0Y-32,4,4,P0Y-32];
     var x=lx[i],y=ly[i],w=70,h=26;
     c.fillStyle=active?'rgba(255,215,0,0.12)':'rgba(0,0,0,0.45)';
     rRect(c,x,y,w,h,3); c.fill();
     c.strokeStyle=active?'#ffd700':'#555'; c.lineWidth=active?2:1;
     rRect(c,x,y,w,h,3); c.stroke();
   ...
   function dCI(c,dora,honba,rb,scores) {{
     var cx=W/2,cy=H/2;
     c.fillStyle='rgba(0,0,0,0.55)'; rRect(c,cx-90,cy-38,180,76,6); c.fill();
     c.strokeStyle='#555'; c.lineWidth=1; rRect(c,cx-90,cy-38,180,76,6); c.stroke();
     c.fillStyle='#ffd700'; c.font='bold 12px system-ui'; c.textAlign='center';
     c.fillText('Dora: '+(dora||[]).join(' '),cx,cy-16);
     c.fillStyle='#aaa'; c.font='11px system-ui';
     c.fillText('\u672c\u573a:'+honba+'  \u7acb\u76f4\u68d2:'+rb,cx,cy+4);
     c.fillStyle='#888'; c.font='10px system-ui';
   ...
   function drawState(canvas,state,active) {{
     ...
     c.clearRect(0,0,W,H);
     c.fillStyle='#1a2a1a'; c.fillRect(0,0,W,H);
     c.fillStyle='#12201a'; rRect(c,8,8,W-16,H-16,12); c.fill();
     if(!state) return;
     ...
     if(state.oya<4) {{ c.fillStyle='#ff9944'; c.font='bold 10px system-ui'; c.textAlign='center'; c.textBaseline='middle'; c.fillText('\u5e84',ox[state.oya],oy[state.oya]); }}
     if(state.last_action) {{ c.fillStyle='#666'; c.font='10px system-ui'; c.textAlign='center'; c.textBaseline='bottom'; c.fillText(state.last_action+' by P'+(state.turn||0),W/2,H-10); }}
   }}

   // ── App ──────────────────────────────────────────────────────────────────
   var Demo=(function(){{
     var steps=DEMO_GAME.steps||[];
     var cur=-1, playing=false, iv=null, speed=600;
     var cv,btnS,btnP,btnR,sN,sT,oEl,hE,rE,dE,spEl,dpE=[];
     function init(){{
       cv=document.getElementById('demo-canvas');
       btnS=document.getElementById('d-btn-start'); btnP=document.getElementById('d-btn-pause');
       btnR=document.getElementById('d-btn-reset');
       sN=document.getElementById('d-step'); sT=document.getElementById('d-total');
       oEl=document.getElementById('d-oya'); hE=document.getElementById('d-honba');
       rE=document.getElementById('d-riichibo'); dE=document.getElementById('d-dora');
       spEl=document.getElementById('d-sv');
     ...
     function render(idx){{
       ...
       if(idx>=0){{
         ...
         oEl.textContent='P'+(st.oya||0); hE.textContent=st.honba||0;
         rE.textContent=st.riichibo||0; dE.textContent=(st.dora||[]).join(' ');
       }}else{{ sN.textContent='0'; oEl.textContent='P0'; hE.textContent='0'; rE.textContent='0'; dE.textContent=''; }}
       for(var i=0;i<4;i++) dpE[i].className='dp'+(i===(st?st.turn:-1)?' active':'');
       btnS.disabled=idx>=steps.length-1;
     }}
     function sched(){{
       clearInterval(iv);
       if(!playing||cur>=steps.length-1) return;
       iv=setInterval(function(){{
         if(cur<steps.length-1){{ step(); }}else{{ pause(); }}
       }},speed);
     }}
     function step(){{
       if(cur<steps.length-1){{ cur++; render(cur); sched(); }}
     }}
     function start(){{ if(cur<0)cur=-1; playing=true; btnS.disabled=true; btnP.disabled=false; sched(); }}
     function pause(){{ playing=false; clearInterval(iv); btnS.disabled=cur>=steps.length-1; btnP.disabled=true; }}
     function reset(){{ pause(); cur=-1; btnS.disabled=false; render(-1); }}
     function setSpeed(v){{ speed=parseInt(v); if(playing) sched(); }}
     init();
...
The live demo above shows a **pre-recorded** 4-AI random battle.
