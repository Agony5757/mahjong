"""Smoke tests for V5 (true Douzero-style) model and encoding strategy."""
...
from pymahjong.rl.v5.action_features import (
    ACTION_FEAT_DIM, build_action_features, torch_action_features,
)
from pymahjong.rl.v5.legal_actions import PAD_INDEX, extract_legal_actions
...
def test_v5_forward_v4compat_shapes_and_masking():
    ...
    m = DouzeroV5Transformer(config=cfg, event_dim=100, scorer_hidden=32)
...
def test_v5_strategy_collate_provides_douzero_tensors():
    """V5Strategy.collate_fn must append all Douzero tensors."""
    strat = get_strategy(EncodingVersion.V5)
    # Build two toy samples in the V4 cached-dataset format.
    sample0 = {
...
def test_v5_strategy_forward_from_batch_uses_douzero():
    strat = get_strategy(EncodingVersion.V5)
    cfg = TransformerConfig(d_model=64, n_layers=2, n_heads=4, ff_mult=2, dropout=0.0)
    model = strat.create_model(transformer_config=cfg, scorer_hidden=32)
    sample0 = {
        "features": torch.zeros((10, 100), dtype=torch.bool),
        "attention_mask": torch.ones(10, dtype=torch.bool),
        "action_mask": torch.zeros(ACTION_DIM, dtype=torch.bool),
    ...
    sample0["action_mask"][[3, 5, 11]] = True
    batch = strat.collate_fn([sample0])
    raw_logits, value, action_mask = strat.forward_from_batch_raw(model, batch)
    assert raw_logits.shape == (1, ACTION_DIM)
    assert value.shape == (1,)
    # Illegal slots must be NEG_INF-ish.
    NEG = -1e8
...
def test_v5_state_dict_roundtrip():
    cfg = TransformerConfig(d_model=64, n_layers=2, n_heads=4, ff_mult=2, dropout=0.0)
    m1 = DouzeroV5Transformer(config=cfg, event_dim=100, scorer_hidden=32)
    m2 = DouzeroV5Transformer(config=cfg, event_dim=100, scorer_hidden=32)
