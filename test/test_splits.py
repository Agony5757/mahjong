...
from pymahjong.rl.cache import ShardWriter, rebuild_manifest
from pymahjong.rl.cached_dataset import CachedEventDataset
from pymahjong.rl.splits import split_by_shard, split_by_track_id
...
def _build_cache(tmp_path: Path, shards: dict[str, int]) -> Path:
    ...
    for name, n in shards.items():
        w = ShardWriter(str(tmp_path / name))
        for i in range(n):
