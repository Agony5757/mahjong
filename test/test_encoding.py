...
class TestEncodingCount:
    """Verify that encode_paipu_file produces exactly 4 tracks per hand."""
    ...
    def test_four_tracks_per_hand(self, paipu_files):
        ...
        from pymahjong.rl.tokenization import encode_paipu_file

        for path in paipu_files:
            samples = encode_paipu_file(path)
            if samples is None:
...
def _count_tracks_per_hand(path: str) -> dict[int, set[int]]:
    ...
    from pymahjong.rl.tokenization import _unsupported_game_type
    ...
    class _FullProxy:
        ...
        def make_selection(self, idx):
            ...
            if enc is not None:
                from pymahjong.rl.tokenization import _route_gamelog_entries
                _route_gamelog_entries(enc, new_entries)
            return ret

    import threading
    from pymahjong.rl.tokenization import _proxy_lock
