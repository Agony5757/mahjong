...
def test_mortal_trainer_lock3train1_mode():
    ...
    # single-learner configuration of the *self-play pool* path; with a
    # fresh (empty) pool no snapshots exist yet, so collection must still
    # succeed.  This path is only active with fixed_bc_opponents=False.
    tcfg = _tiny_cfg()
    cfg = MortalConfig(
        total_steps=40, rollout_steps=30, n_envs=1, n_epochs=1, batch_size=16,
        fixed_bc_opponents=False,
...
def test_fixed_bc_opponents_seat_rotation():
    # In fixed-BC mode _assign_seats rotates the learner seat 0->1->2->3 and
    # the opponent seats are always the complementary three.
    tcfg = _tiny_cfg()
    cfg = MortalConfig(fixed_bc_opponents=True, save_path="", device="cpu",
                       scorer_hidden=32, seed=0)
    tr = MortalTrainer(config=cfg, transformer_config=tcfg)
    seen = []
    for _ in range(4):
        opp_assign, opp_seats, learner_seat = tr._assign_seats(env_seed=0)
        assert opp_assign == {}, "fixed-BC mode uses no pool snapshots"
        assert opp_seats == {0, 1, 2, 3} - {learner_seat}
        seen.append(learner_seat)
    assert seen == [0, 1, 2, 3], f"learner seat must rotate 0..3, got {seen}"


def test_fixed_bc_opponents_are_frozen():
    # The 3 frozen-BC opponents must never be updated by training, while the
    # learner's parameters do change.
    tcfg = _tiny_cfg()
    cfg = MortalConfig(
        total_steps=40, rollout_steps=30, n_envs=2, n_epochs=1, batch_size=16,
        reward_kind="points", fixed_bc_opponents=True, snapshot_interval=10**9,
        save_path="", save_interval=10**9, seed=0, device="cpu",
    )
    tr = MortalTrainer(config=cfg, transformer_config=tcfg)
    assert tr._bc_opponent is not None
    assert all(not p.requires_grad for p in tr._bc_opponent.parameters())
    opp_before = [p.detach().clone() for p in tr._bc_opponent.parameters()]
    learner_before = [p.detach().clone() for p in tr.model.parameters()]
    tr.train()
    assert all(torch.equal(b, a)
               for b, a in zip(opp_before, tr._bc_opponent.parameters())), \
        "frozen BC opponents must not change during training"
    assert any(not torch.equal(b, a)
               for b, a in zip(learner_before, tr.model.parameters())), \
        "learner parameters must update"


def test_bc_anchor_ce_equals_entropy_when_learner_equals_bc():
    # The BC anchor is CE(learner policy, BC soft labels) = forward
    # KL(BC || learner) + H(BC).  When learner == BC the KL is 0, so the
    # anchor CE must equal the entropy of the BC policy.
    torch.manual_seed(0)
    m = MortalQNet(_tiny_cfg(), scorer_hidden=32).eval()
    bc = MortalQNet(_tiny_cfg(), scorer_hidden=32).eval()
    bc.load_state_dict(m.state_dict())  # identical reference
    obs = [_make_obs([0, 1, 5, 37], batch_seqlen=4),
           _make_obs([2, 3, 4], batch_seqlen=4)]
    feats = torch.stack([torch.tensor(o["features"], dtype=torch.float32) for o in obs])
    attn = torch.stack([torch.tensor(o["attention_mask"], dtype=torch.bool) for o in obs])
    amask = torch.stack([torch.tensor(o["action_mask"], dtype=torch.bool) for o in obs])
    actions = torch.tensor([0, 2], dtype=torch.long)
    with torch.no_grad():
        out = m.evaluate_q(feats, attn, amask, actions)
        _h, bc_qK, _p, _l, _t = bc._q_legal(feats, attn, amask, actions=None)
        bc_p = torch.softmax(bc_qK, dim=-1)
        learner_logp = torch.log_softmax(out["q_K"], dim=-1)
        pad = out["action_pad_mask"]
        ce = -(bc_p * learner_logp).masked_fill(~pad, 0.0).sum(-1)
        ent = -(bc_p * torch.log(bc_p.clamp_min(1e-12))).masked_fill(~pad, 0.0).sum(-1)
    assert torch.allclose(ce, ent, atol=1e-4), f"KL(BC||learner)!=0: ce={ce}, ent={ent}"


def test_bc_anchor_trains_without_nan_and_reuses_opponent():
    tcfg = _tiny_cfg()
    cfg = MortalConfig(
        total_steps=40, rollout_steps=30, n_envs=2, n_epochs=1, batch_size=16,
        reward_kind="points", fixed_bc_opponents=True, bc_anchor_weight=1.0,
        snapshot_interval=10**9, save_path="", save_interval=10**9,
        seed=0, device="cpu",
    )
    tr = MortalTrainer(config=cfg, transformer_config=tcfg)
    assert tr._bc_anchor is not None
    assert tr._bc_anchor is tr._bc_opponent, "anchor should reuse the fixed-BC opponent"
    before = [p.detach().clone() for p in tr.model.parameters()]
    tr.train()
    assert all(torch.isfinite(p).all() for p in tr.model.parameters()), "NaN/Inf in params"
    assert any(not torch.equal(b, a) for b, a in zip(before, tr.model.parameters())), \
        "learner parameters must update"


def test_huber_dqn_loss_path_runs():
    tcfg = _tiny_cfg()
    cfg = MortalConfig(
        total_steps=40, rollout_steps=30, n_envs=2, n_epochs=1, batch_size=16,
        reward_kind="points", fixed_bc_opponents=True, bc_anchor_weight=2.0,
        huber_delta=1.0, snapshot_interval=10**9, save_path="",
        save_interval=10**9, seed=0, device="cpu",
    )
    tr = MortalTrainer(config=cfg, transformer_config=tcfg)
    tr.train()
    assert all(torch.isfinite(p).all() for p in tr.model.parameters()), "NaN/Inf with Huber"
